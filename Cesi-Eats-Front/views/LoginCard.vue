  ...
  <div class="bg-surface-50 dark:bg-surface-950 px-6 py-20 md:px-12 lg:px-20">
    <div class="bg-surface-0 dark:bg-surface-900 p-6 shadow rounded-border w-full lg:w-6/12 mx-auto">
      <div class="text-center mb-8">
        ...
        <span class="text-surface-600 dark:text-surface-200 font-medium leading-normal">Pas de compte ?</span>
      ...
      <div>
        ...
        <InputText id="email1" type="text" placeholder="Adresse email" class="w-full mb-4" />

        <label for="password1" class="text-surface-900 dark:text-surface-0 font-medium mb-2 block">Mot de passe</label>
        <InputText id="password1" type="password" placehoder="Mot de passe" class="w-full mb-4" />

        <div class="flex items-center justify-between mb-12">
          <a class="font-medium no-underline ml-2 text-primary text-right cursor-pointer">Forgot password?</a>
        </div>

        <router-link label="Sign In" icon="pi pi-user !text-xl !leading-none" class="w-full nav-link ml-auto" to="/sign"> Sign In</router-link>
      </div>
    </div>
  </div>
</template>
